...
const ShippingAddresses = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingId) {
                await checkoutAPI.updateShippingAddress(editingId, formData);
                alert('Address updated successfully');
            } else {
                await checkoutAPI.createShippingAddress(formData);
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this address?')) {
            try {
                await checkoutAPI.deleteShippingAddress(id);
                fetchAddresses();
            } catch (error) {
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
            ...
            {addresses.length === 0 && !showForm && (
                <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 text-lg">No shipping addresses saved yet.</p>
                    <button
                        onClick={() => setShowForm(true)}
